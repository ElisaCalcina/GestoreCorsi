...
public class CorsoDAO {
	...
	public boolean esisteCorso(String codins) {
		...
		try {
			...
			if(rs.next()) {
				conn.close(); //PRIMA DI OGNI RETURN DEVO CHIUDERE LA CONNESSIONE
				return true;
			} else {
				conn.close();
				return false;
	...
	public Map<Corso, Integer> getIscrittiByPeriodo(Integer pd){
		...
		try {
			Connection conn= ConnectDB.getConnection();
			PreparedStatement st= conn.prepareStatement(sql);
			st.setInt(1, pd); //riempio il punto interrogativo
	...
	public List<Studente> getStudentiByCorso(Corso corso){
		...
		try {
			Connection conn= ConnectDB.getConnection();
			PreparedStatement st= conn.prepareStatement(sql);
			st.setString(1, corso.getCodins()); //dato che ho oggetto corso (Corso corso) devo usare questo per recuperare il codins
			ResultSet rs=st.executeQuery();
	...
	//aggiungo metodo per soluzione 2
